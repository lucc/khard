...
def list_contacts(selected_addressbooks, vcard_list):
    if selected_addressbooks.__len__() == 1:
        print "Addressbook: %s" % selected_addressbooks[0]
        table = [["Id", "Name", "Phone", "E-Mail"]]
    else:
        print "Addressbooks: %s" % ', '.join(selected_addressbooks)
        table = [["Id", "Name", "Phone", "E-Mail", "Addressbook"]]
...
parser = argparse.ArgumentParser(description="Carddav manager")
parser.add_argument("-a", "--addressbook", default=','.join(addressbooks.keys()),
        help="Specify addressbook names as comma separated list")
...
parser.add_argument("action", nargs="?", default=config.get_default_action(),
        help="Possible actions: list, details, mutt, new, modify and remove")
args = parser.parse_args()

# version
if args.version == True:
    print "Khard version 0.1"
    sys.exit(0)

# validate value for action
if args.action == "":
    print "Missing action. Possible values are: list, details, mutt, new, modify and remove"
    sys.exit(1)
elif args.action not in ["list", "details", "mutt", "new", "modify", "remove"]:
    print "Unsupported action. Possible values are: list, details, mutt, new, modify and remove"
    sys.exit(1)

# given addressbook name
selected_addressbooks = []
for name in args.addressbook.split(","):
    if config.has_addressbook(name) == False:
        print "Error: The entered addressbook \"%s\" does not exist. Possible values are: %s" \
...
if args.action == "new":
    if selected_addressbooks.__len__() != 1:
        print "Error: You must specify one addressbook, in which the new \
